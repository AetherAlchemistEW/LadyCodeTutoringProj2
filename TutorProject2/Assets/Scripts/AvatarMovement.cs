...
public class AvatarMovement : MonoBehaviour
{
    ...
    IEnumerator MoveToPoint()
    {
        ...
        while (ourPath.Count > 0) //This loop will run as long as we have nodes in our path list, we're handing waypointing a bit differently this time.
        {
            //while we're not near the current (in this case, first) waypoint
            while (Vector3.Distance(transform.position, ourPath[0]) > 1) 
            {
